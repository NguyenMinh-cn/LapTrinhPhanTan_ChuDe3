...
public class GiaoDienLamBaiThi extends JPanel {
    ...
    private BaiThiCallback thongBaoCallback;

    private class ThongBaoBaiThiMoiImpl extends UnicastRemoteObject implements BaiThiCallback {
        protected ThongBaoBaiThiMoiImpl() throws RemoteException {
            super();
        }
        @Override
        public void nhanThongBaoBaiThiMoi(BaiThi baiThi) throws RemoteException {
            if (baiThi != null && baiThi.getDanhSachLop().stream()
                    .anyMatch(lop -> lop.getMaLop() == hocSinh.getLop().getMaLop())) {
                SwingUtilities.invokeLater(() -> {
                    try {
                        capNhatDanhSachBaiThi();
                        JOptionPane.showMessageDialog(GiaoDienLamBaiThi.this,
                                "Có bài thi mới được thêm: " + baiThi.getTenBaiThi(),
                                "Thông báo",
    ...
    public GiaoDienLamBaiThi(HocSinh hocSinh) throws MalformedURLException, NotBoundException, RemoteException {
        ...
        thongBaoCallback = new ThongBaoBaiThiMoiImpl();
        baiThiService.dangKyClientNhanThongBao(thongBaoCallback);
    ...
    private void capNhatDanhSachBaiThi() throws RemoteException, MalformedURLException, NotBoundException {
        taoDSBaiThi();
    }
    ...
    public void donDep() {
        try {
            if (thongBaoCallback != null) {
                baiThiService.huyDangKyClientNhanThongBao(thongBaoCallback);
            }
